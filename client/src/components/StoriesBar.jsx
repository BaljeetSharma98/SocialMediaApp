...
const StoriesBar = () => {
  const { stories, fetchStories } = useApp();
    ...
    <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4">
      <div className="flex gap-4 pb-5">
        ...
        <div onClick={()=> setShowModal(true)}
          className="rounded-lg shadow-sm min-w-[120px] max-w-[120px] aspect-[3/4] cursor-pointer hover:shadow-lg transition-all duration-200
        ...
        {stories.map((story, index) => (
          <div onClick={()=> setViewStory(story)}
            key={index}
            className="relative rounded-lg shadow min-w-[120px] max-w-[120px] aspect-[3/4] cursor-pointer hover:shadow-lg
          ...
          >
            ...
            />
            <p className="absolute bottom-6 left-3 text-white/80 text-sm truncate max-w-[100px]">
              {story.content}
            </p>
